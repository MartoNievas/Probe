...
typedef struct {
  uint32_t id;    // asmdef_offset:USUARIO_ID_OFFSET 0
  uint32_t nivel; // asmdef_offset:USUARIO_NIVEL_OFFSET 4
} usuario_t;      // asmdef_size:USUARIO_SIZE 8

typedef struct {
  char categoria[3];  // asmdef_offset:CASO_CATEGORIA_OFFSET 0
  uint16_t estado;    // asmdef_offset:CASO_ESTADO_OFFSET 4
  usuario_t *usuario; // asmdef_offset:CASO_USUARIO_OFFSET 8
} caso_t;             // asmdef_size:CASO_SIZE 16

typedef struct {
  caso_t *casos_nivel_0; // asmdef_offset:SEGMENTACION_CASOS0_OFFSET 0
  caso_t *casos_nivel_1; // asmdef_offset:SEGMENTACION_CASOS1_OFFSET 8
  caso_t *casos_nivel_2; // asmdef_offset:SEGMENTACION_CASOS2_OFFSET 16
} segmentacion_t;        // asmdef_size:SEGMENTACION_SIZE 24

typedef struct {
  uint8_t cantidad_CLT;      // asmdef_offset:ESTADISTICAS_CLT_OFFSET 0
  uint8_t cantidad_RBO;      // asmdef_offset:ESTADISTICAS_RBO_OFFSET 1
  uint8_t cantidad_KSC;      // asmdef_offset:ESTADISTICAS_KSC_OFFSET 2
  uint8_t cantidad_KDT;      // asmdef_offset:ESTADISTICAS_KDT_OFFSET 3
  uint8_t cantidad_estado_0; // asmdef_offset:ESTADISTICAS_ESTADO0_OFFSET 4
  uint8_t cantidad_estado_1; // asmdef_offset:ESTADISTICAS_ESTADO1_OFFSET 5
  uint8_t cantidad_estado_2; // asmdef_offset:ESTADISTICAS_ESTADO2_OFFSET 6
} estadisticas_t;            // asmdef_size:ESTADISTICAS_SIZE 7
