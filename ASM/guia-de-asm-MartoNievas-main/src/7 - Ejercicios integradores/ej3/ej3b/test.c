 ...
 */
bool verificar_casos_procesados(funcionCierraCasos_t *funcion, caso_t *casos,
                                int largo, caso_t *casos_a_revisar,
                                int largo_revisar) {
  // Verificar que los casos de nivel 0 van a revisión
  // Verificar que los casos de nivel 1 y 2 con resultado 0 y categoría CLT/RBO
  // se cierran desfavorablemente Verificar que los casos de nivel 1 y 2 con
  // resultado 1 se cierran favorablemente Verificar que los casos de nivel 1 y
  // 2 con resultado 0 y categoría diferente van a revisión
  ...
  for (int i = 0; i < largo; i++) {
    ...
    if (nivel == 0) {
      ...
      if (resultado == 1) {
        ...
        // Verificar categoría
        if (strcmp(casos[i].categoria, "CLT") == 0 ||
            strcmp(casos[i].categoria, "RBO") == 0) {
 ...
 */
int contar_casos_a_revision(funcionCierraCasos_t *funcion, caso_t *casos,
                            int largo) {
  int contador = 0;
...
TEST(ej2_resolver_automaticamente) {
  ...
  caso_t casos_test1[2] = {crear_caso(1, 0, "A1", 1),
                           crear_caso(2, 0, "B2", 2)};

  caso_t casos_a_revisar1[2];
  TEST_CALL_V(resolver_automaticamente, funcionCierraCasos, casos_test1,
              casos_a_revisar1, 2);
  ...
  if (casos_test1[0].estado != 1 || casos_test1[1].estado != 2) {
    failed = true;
    printf("\n" FAIL "ej2_resolver_automaticamente_nivel_0: casos de nivel 0 "
                     "no mantuvieron su estado\n");
  }

  // chequeo de integridad
  caso_t casos_test1_copia[2] = {crear_caso(1, 0, "A1", 1),
                                 crear_caso(2, 0, "B2", 2)};

  for (int i = 0; i < 2; i++) {
    if (!casos_iguales_excepto_estado(casos_test1[i], casos_test1_copia[i])) {
      failed = true;
      printf("\n" FAIL "ej2_resolver_automaticamente_nivel_0: Se modificaron "
                       "datos de entrada\n");
      break;
    }
  }
  ...
  TEST_CALL_V(resolver_automaticamente, funcionCierraCasos, casos_test2,
              casos_a_revisar2, 3);

  failed = false;

  // Verificar que los casos con resultado 1 se cerraron favorablemente (estado
  // = 1)
  if (casos_test2[0].estado != 1 || casos_test2[1].estado != 1 ||
      casos_test2[2].estado != 1) {
    failed = true;
    printf(FAIL "ej2_resolver_automaticamente_cierre_favorable: casos no se "
                "cerraron favorablemente\n");
  }
  ...
  for (int i = 0; i < 3; i++) {
    if (!casos_iguales_excepto_estado(casos_test2[i], casos_test2_copia[i])) {
      failed = true;
      printf(FAIL "ej2_resolver_automaticamente_cierre_favorable: Se "
                  "modificaron datos de entrada\n");
      break;
    }
  }
  ...
  // Test 3: Casos de nivel 1 y 2 con resultado 0 y categoría CLT/RBO (cierre
  // desfavorable)
  ...
  TEST_CALL_V(resolver_automaticamente, funcionCierraCasos, casos_test3,
              casos_a_revisar3, 4);

  failed = false;

  // Verificar que los casos con resultado 0 y categoría CLT/RBO se cerraron
  // desfavorablemente (estado = 2)
  if (casos_test3[0].estado != 2 || casos_test3[1].estado != 2 ||
      casos_test3[2].estado != 2 || casos_test3[3].estado != 2) {
    failed = true;
    printf(FAIL "ej2_resolver_automaticamente_cierre_desfavorable: casos no se "
                "cerraron desfavorablemente\n");
  }
  ...
  for (int i = 0; i < 4; i++) {
    if (!casos_iguales_excepto_estado(casos_test3[i], casos_test3_copia[i])) {
      failed = true;
      printf(FAIL "ej2_resolver_automaticamente_cierre_desfavorable: Se "
                  "modificaron datos de entrada\n");
      break;
    }
  }
  ...
  // Test 4: Casos de nivel 1 y 2 con resultado 0 y categoría diferente (van a
  // revisión)
  ...
  TEST_CALL_V(resolver_automaticamente, funcionCierraCasos, casos_test4,
              casos_a_revisar4, 3);

  failed = false;

  // Verificar que los casos con resultado 0 y categoría diferente mantienen su
  // estado original
  if (casos_test4[0].estado != 2 || casos_test4[1].estado != 4 ||
      casos_test4[2].estado != 6) {
    failed = true;
    printf(FAIL "ej2_resolver_automaticamente_van_a_revision: casos no "
  ...
  for (int i = 0; i < 3; i++) {
    if (!casos_iguales_excepto_estado(casos_test4[i], casos_test4_copia[i])) {
      failed = true;
      printf(FAIL "ej2_resolver_automaticamente_van_a_revision: Se modificaron "
                  "datos de entrada\n");
      break;
    }
  }
  ...
  caso_t casos_test5[6] = {
      ...
      crear_caso(6, 2, "XYZ",
                 6),            // ID par, categoría diferente -> va a revisión
      crear_caso(5, 1, "C5", 5) // ID impar -> cierre favorable
  };

  caso_t casos_a_revisar5[6];
  TEST_CALL_V(resolver_automaticamente, funcionCierraCasos, casos_test5,
              casos_a_revisar5, 6);
  ...
  if (casos_test5[0].estado != 1) { // Nivel 0 mantiene estado
    ...
    printf(
        FAIL
        "ej2_resolver_automaticamente_mixto: caso nivel 0 no mantuvo estado\n");
  }
  if (casos_test5[1].estado != 1) { // Cierre favorable
    failed = true;
    printf(FAIL "ej2_resolver_automaticamente_mixto: caso cierre favorable "
                "incorrecto\n");
  }
  if (casos_test5[2].estado != 2) { // Cierre desfavorable CLT
    failed = true;
    printf(FAIL "ej2_resolver_automaticamente_mixto: caso cierre desfavorable "
                "CLT incorrecto\n");
  }
  ...
  if (casos_test5[4].estado != 6) { // Va a revisión
    failed = true;
    printf(FAIL "ej2_resolver_automaticamente_mixto: caso va a revisión no "
                "mantuvo estado\n");
  }
  ...
  for (int i = 0; i < 6; i++) {
    if (!casos_iguales_excepto_estado(casos_test5[i], casos_test5_copia[i])) {
      failed = true;
      printf(FAIL "ej2_resolver_automaticamente_mixto: Se modificaron datos de "
                  "entrada\n");
      break;
    }
  }
  ...
  caso_t casos_test7[4] = {
      ...
      crear_caso(4294967295, 2, "RBO",
                 2),              // ID máximo (impar) -> cierre favorable
  ...
  TEST_CALL_V(resolver_automaticamente, funcionCierraCasos, casos_test7,
              casos_a_revisar7, 4);
  ...
  if (casos_test7[0].estado != 2) { // ID 0 (par) con CLT -> cierre desfavorable
    failed = true;
    printf(FAIL
           "ej2_resolver_automaticamente_valores_extremos: ID 0 incorrecto\n");
  }
  if (casos_test7[1].estado != 1) { // ID máximo (impar) -> cierre favorable
    failed = true;
    printf(FAIL "ej2_resolver_automaticamente_valores_extremos: ID máximo "
                "incorrecto\n");
  }
  if (casos_test7[2].estado != 1) { // ID 1 (impar) -> cierre favorable
    failed = true;
    printf(FAIL
           "ej2_resolver_automaticamente_valores_extremos: ID 1 incorrecto\n");
  }
  if (casos_test7[3].estado !=
      4) { // ID grande (par) con categoría diferente -> mantiene estado
    failed = true;
    printf(FAIL "ej2_resolver_automaticamente_valores_extremos: ID grande "
                "incorrecto\n");
  }
  ...
  caso_t casos_test7_copia[4] = {
      crear_caso(0, 1, "CLT", 1), // ID mínimo (par) -> cierre desfavorable
      crear_caso(4294967295, 2, "RBO",
                 2),              // ID máximo (impar) -> cierre desfavorable
      crear_caso(1, 1, "ABC", 3), // ID pequeño (impar) -> cierre favorable
      crear_caso(999999998, 2, "XYZ", 4) // ID grande (par) -> va a revisión
  };
  ...
  for (int i = 0; i < 4; i++) {
    if (!casos_iguales_excepto_estado(casos_test7[i], casos_test7_copia[i])) {
      failed = true;
      printf(FAIL "ej2_resolver_automaticamente_valores_extremos: Se "
                  "modificaron datos de entrada\n");
      break;
    }
  }
  ...
  TEST_CALL_V(resolver_automaticamente, funcionCierraCasos, casos_test8,
              casos_a_revisar8, 3);
  ...
  if (casos_test8[0].estado != 2) { // Debe cambiar a cierre desfavorable
    failed = true;
    printf(FAIL "ej2_resolver_automaticamente_estados_extremos: estado mínimo "
                "incorrecto\n");
  }
  if (casos_test8[1].estado != 2) { // Debe cambiar a cierre desfavorable
    failed = true;
    printf(FAIL "ej2_resolver_automaticamente_estados_extremos: estado máximo "
                "incorrecto\n");
  }
  if (casos_test8[2].estado != 32767) { // Debe mantener estado (va a revisión)
    failed = true;
    printf(FAIL "ej2_resolver_automaticamente_estados_extremos: estado medio "
                "incorrecto\n");
  }
  ...
  for (int i = 0; i < 3; i++) {
    if (!casos_iguales_excepto_estado(casos_test8[i], casos_test8_copia[i])) {
      failed = true;
      printf(FAIL "ej2_resolver_automaticamente_estados_extremos: Se "
                  "modificaron datos de entrada\n");
      break;
    }
  }
  ...
  TEST_CALL_V(resolver_automaticamente, funcionCierraCasos, casos_test9,
              casos_a_revisar9, 6);
  ...
  if (casos_test9[0].estado != 2) { // CLT exacto -> cierre desfavorable
    failed = true;
    printf(FAIL "ej2_resolver_automaticamente_categorias_edge: CLT exacto "
                "incorrecto\n");
  }
  if (casos_test9[1].estado != 2) { // RBO exacto -> cierre desfavorable
    failed = true;
    printf(FAIL "ej2_resolver_automaticamente_categorias_edge: RBO exacto "
                "incorrecto\n");
  }
  if (casos_test9[2].estado != 3) { // clt minúsculas -> mantiene estado
    failed = true;
    printf(FAIL "ej2_resolver_automaticamente_categorias_edge: clt minúsculas "
                "incorrecto\n");
  }
  if (casos_test9[3].estado != 4) { // rbo minúsculas -> mantiene estado
    failed = true;
    printf(FAIL "ej2_resolver_automaticamente_categorias_edge: rbo minúsculas "
                "incorrecto\n");
  }
  if (casos_test9[4].estado != 5) { // CLTX -> mantiene estado
    failed = true;
    printf(FAIL
           "ej2_resolver_automaticamente_categorias_edge: CLTX incorrecto\n");
  }
  if (casos_test9[5].estado != 6) { // RBOY -> mantiene estado
    failed = true;
    printf(FAIL
           "ej2_resolver_automaticamente_categorias_edge: RBOY incorrecto\n");
  }
  ...
  caso_t casos_test10[10] = {
      ...
      crear_caso(3, 1, "B3",
                 3), // 8 casos nivel 1 (todos impares -> cierre favorable)
      crear_caso(5, 1, "B5", 5),
      crear_caso(7, 1, "B7", 7),
      crear_caso(9, 1, "B9", 9),
      ...
      crear_caso(2, 2, "CLT",
                 2) // 1 caso nivel 2 (par con CLT -> cierre desfavorable)
  };

  caso_t casos_a_revisar10[10];
  TEST_CALL_V(resolver_automaticamente, funcionCierraCasos, casos_test10,
              casos_a_revisar10, 10);
  ...
  if (casos_test10[0].estado != 1) { // Nivel 0 mantiene estado
    failed = true;
    printf(FAIL "ej2_resolver_automaticamente_distribucion_desbalanceada: "
                "nivel 0 incorrecto\n");
  }

  // Verificar que todos los casos de nivel 1 (impares) se cerraron
  // favorablemente
  for (int i = 1; i < 9; i++) {
  ...
  if (casos_test10[9].estado !=
      2) { // Nivel 2 (par con CLT) -> cierre desfavorable
    failed = true;
    printf(FAIL "ej2_resolver_automaticamente_distribucion_desbalanceada: "
                "último caso incorrecto\n");
  }
  ...
  for (int i = 0; i < num_casos; i++) {
    casos_test11[i] = crear_caso(i, i % 3,
                                 (i % 4 == 0)   ? "CLT"
                                 : (i % 4 == 1) ? "RBO"
                                                : "ABC",
                                 i);
  ...
  TEST_CALL_V(resolver_automaticamente, funcionCierraCasos, casos_test11,
              casos_a_revisar11, num_casos);

  failed = false;

  // Verificar que se procesaron correctamente
  int casos_esperados_revision =
      contar_casos_a_revision(funcionCierraCasos, casos_test11, num_casos);

  if (casos_esperados_revision < 0) {
    failed = true;
    printf(FAIL "ej2_resolver_automaticamente_memoria_grande: error en conteo "
                "de casos\n");
  }
