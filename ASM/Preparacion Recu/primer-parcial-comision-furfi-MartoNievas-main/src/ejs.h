...
typedef struct tuit_s {
  char mensaje[140];       // asmdef_offset:TUIT_MENSAJE_OFFSET 0
  uint16_t favoritos;      // asmdef_offset:TUIT_FAVORITOS_OFFSET 140
  uint16_t retuits;        // asmdef_offset:TUIT_RETUITS_OFFSET 142
  uint32_t id_autor;       // asmdef_offset:TUIT_ID_AUTOR_OFFSET 144
} tuit_t;                  // asmdef_offset:TUIT_SIZE 148

// Estructura publicacion
typedef struct publicacion_s {
  publicacion_t *next;     // asmdef_offset:PUBLICACION_NEXT_OFFSET 0
  tuit_t *value;           // asmdef_offset:PUBLICACION_VALUE_OFFSET 8
} publicacion_t;           // asmdef_offset:PUBLICACION_SIZE 16
...
typedef struct usuario_s {
  feed_t *feed;            // asmdef_offset:USUARIO_FEED_OFFSET 0
  usuario_t **seguidores;  // asmdef_offset:USUARIO_SEGUIDORES_OFFSET 8
